...
def test_run_local_script_is_executable_and_exposes_help():
    ...
    assert "POSTARA_HOST" in result.stdout
    assert "POSTARA_PORT" in result.stdout
...
def test_run_local_script_loads_env_migrates_and_runs_uvicorn():
    ...
    assert '"$PYTHON_BIN" -m uvicorn postara.api:app' in script
    assert 'POSTARA_PORT="${POSTARA_PORT:-18080}"' in script
